...
class Axon(object):
    ...
    def simulate(self, rec_imem=True, index=0):
        ...
        t_axonSimInitDone = time.time()

        h.run()

        t_axonMainSimDone = time.time()

        if rec_imem:
            self.calc_imem()

        t_axonCalImemDone = time.time()

        t_simInit = t_axonSimInitDone - t_axonSimStart
        t_mainSim = t_axonMainSimDone - t_axonSimInitDone
        t_imem = t_axonCalImemDone - t_axonMainSimDone
        return (t_simInit, t_mainSim, t_imem)
